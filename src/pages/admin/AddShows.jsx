    ...
    <div className="mt-10 flex flex-col gap-6"> </div>
            ...
            <div key={movie.id} className={`relative max-w-40 cursor-pointer
              ...
              duration-300`} onClick={()=> setselectedMovies(movie.id)}>
                <div className="relative rounded-lg overflow-hidden">
                  <img src={movie.show.movie.poster_path} alt="" className="w-full 
                  object-cover brightness-90" />
                  <div className="text-sm flex items-center jjustify-between
                  p-2 bg-black/70 w-full absolute bottom-0 left-0">
                    ...
                    <p className="text-gray-300">{KConverter(movie.show.movie.vote_count)} Votes</p>
                   </div>
                  </div>
                  ...
                  <p className="font-medium truncate">{movie.show.movie.title}</p>
                  <p className="text-gray-400 text-sm">{movie.show.movie.release_date}</p>
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="mt-8">
        ...
        px-3 py-2 rounded-md">
          ...
          <input min={0} type="number" vlaue={showPrice} onChange={(e)=>
      ...
      <div className="mt-6">
        ...
        <div className="inline-flex gap-2 border border-gray-600 p-1 pl-1
        rounded-lg h-15">
          <input type="datetime-local" value={dateTimeInput} onChange={(e)=>
            setDateTimeInput(e.target.value)} className="outline-none
            rounded-md " />
            <button onClick={handleDateTimeAdd} className="bg-red-400/80
            text-white px-2 py-2 text-sm rounded-lg hover:bg-primary
            cursor-pointer">
              Add Time
            </button>
      ...
      <div>
      <button className="bg-red-400 text-white px-4 py-0.5 mt-6 rounded
      hover:bg-red-500 transition-all cursor-pointer">
        Add show
      </button>
      </div>
    </>
